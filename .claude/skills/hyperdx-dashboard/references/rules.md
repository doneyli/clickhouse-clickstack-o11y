...
| 5 | `aggFn` must be valid | **Standard (public + internal):** `count`, `count_rate`, `sum`, `avg`, `min`, `max`, `p50`, `p90`, `p95`, `p99`, `count_distinct`, `avg_rate`, `sum_rate`, `min_rate`, `max_rate`, `p50_rate`, `p90_rate`, `p95_rate`, `p99_rate`. **Internal API only:** `last_value`, `count_per_sec`, `count_per_min`, `count_per_hour`. Invalid values fail silently. |
...
| 14 | Always emit: `seriesReturnType: "column"` on every chart, `table: "logs"` (or `"metrics"` for metric data) on every series, `groupBy: []` on every `time` series, `query: ""` at dashboard level | Omitting these creates non-deterministic API behavior. |
